...
* 排查模型调用问题，给每个模型对应的调用规范，并给出调用示例
* 和后端沟通接口调用规范，完成接口封装
* 按需要修改对应的模型加载代码
* 日志应当优化，先放一下，优先完成接口封装

###### 遇到的问题

1. 得到很全部模型的输出，检查发现模型大小不同但是参数设置完全一致，问题定位到model_loader.py 142: model_instance.load_state_dict(state_dict, strict=False)，目前推测是迁移学习导致不同模型中相同的参数被用到的resnet中
2. 日志会重复输出，暂时不排查问题所在
